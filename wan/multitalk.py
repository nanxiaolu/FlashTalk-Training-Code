...
class InfiniteTalkPipeline:
    ...
    def generate_infinitetalk(self,
        ...
        gen_video_list = []
        early_stop_step = getattr(extra_args, 'early_stop_step', None)
        prev_clip_diff = None
        torch_gc()
        ...
        while True:
            ...
            with torch.no_grad():
                ...
                # Keep reference latent fixed after first clip to reduce long-video drift.
                # Only apply this when use_fixed_reference_latent is enabled
                if getattr(extra_args, 'use_fixed_reference_latent', False):
                    if (not is_first_clip) and (fixed_reference_latent is not None):
                        y[:, :, :1] = fixed_reference_latent.to(device=y.device, dtype=y.dtype)
            ...
            with torch.no_grad(), no_sync():
                ...
                # prepare timestep
                if getattr(extra_args, 'dmd_inference', False):
                    # DMD 推理：4 步时用 [1000, 750, 500, 250]，k 步取前 k 个（k<=4）
                    if sampling_steps == 4:
                        timesteps = [1000, 750, 500, 250]
                    elif sampling_steps == 3:
                        timesteps = [1000, 750, 500]
                    elif sampling_steps == 2:
                        timesteps = [1000, 750]
                    elif sampling_steps == 1:
                        timesteps = [1000]
                    print(f"DMD inference mode, sampling_steps={sampling_steps}, timesteps={timesteps}")
                else:    
                ...
                for i in progress_wrap(range(len(timesteps)-1)):
                    ...
                    # Check if CFG is disabled
                    if getattr(extra_args, 'disable_cfg', False):
                        # No CFG: use only conditional prediction
                        noise_pred = noise_pred_cond
                    else:
                        # CFG enabled: compute unconditional predictions
                        if math.isclose(text_guide_scale, 1.0):
                            noise_pred_drop_audio = self.model(
                                latent_model_input, t=timestep, **arg_null_audio)[0]  
                            torch_gc()
                        elif math.isclose(audio_guide_scale, 1.0):
                    ...
                    # Early stop: at the specified step, take a big Euler jump to t=0 and stop
                    if early_stop_step is not None and i == early_stop_step:
                        dt_big = timesteps[i] / self.num_timesteps
                        latent = latent + noise_pred * dt_big[:, None, None, None]
                        latent[:, :cur_motion_frames_latent_num] = latent_motion_frames
                        x0 = [latent.to(self.device)]
                        logging.info(f"Early stop at step {i}: big jump from t={timesteps[i].item():.1f} to t=0")
                ...
                # Save the first frame latent only if use_fixed_reference_latent is enabled
                if getattr(extra_args, 'use_fixed_reference_latent', False):
                    if is_first_clip and fixed_reference_latent is None:
                        fixed_reference_latent = x0[0][:, :1].detach().clone()
