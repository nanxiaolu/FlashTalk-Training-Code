...
#
# IMPORTANT: ``--group_size`` MUST equal the number of GPUs used at stage-2 training time. 
# Stage-2 trains under FSDP with self-forcing++ chunked
...
python tools/payload_files_to_lmdb.py \
    ...
    --group_size 8 # group_size MUST equal the number of GPUs used at stage-2 training time
